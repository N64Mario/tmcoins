            ...
            if($code){
                ...
                if($pages == 'deal'){
                    ...
                    $money['ex_sell_min'] = $productInfo['ex_sell_min'];
                     ...
                     $code = 'usdt';
                    $productInfo = \app\admin\model\ProductLists::where('code',$code)->field('id,title,close,le_sx_fee,le_day_sx_fee,le_no_sx_fee,le_play_time,le_play_type,le_order_rate,last_price')->find();
                    $money['money'] = \app\admin\model\MemberWallet::where('product_id',$productInfo['id'])->where('uid',$this->memberInfo['id'])->value('le_money')*$productInfo['last_price'];
                    $money['pro_tit'] = $productBase['title'];
                    $money['le_play_time'] = $productInfo['le_play_time'];
                    $money['le_sx_fee'] = $productInfo['le_sx_fee'];
                    $money['le_sx_fee_100'] =$productInfo['le_sx_fee'].'%';
                    $money['le_order_rate'] = $productInfo['le_order_rate'];
                }else if($pages == 'seconds'){
                    $productInfo = \app\admin\model\ProductLists::where('code',$code)->field('id,title,close,op_sx_fee,op_play_time,op_play_price,op_play_prop,op_play_max,op_order_min,op_order_max,op_play_down')->find();
                    ...
                    $money['op_play_down'] = explode(',',$productInfo['op_play_down']);
