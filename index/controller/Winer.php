...
class Winer extends IndexController
{
    ...
    public function get_num()
    {
        if(request()->isPost()){
            ...
            $info = [];
            if($winer_id && $num){
                $winer = \app\admin\model\MinerLists::where('id',$winer_id)
                    ->field('product_id,max_rate')->find();
                $pro = \app\admin\model\ProductLists::where('id',$winer['product_id'])
                    ->field('title,close')->find();
                //价格换算
