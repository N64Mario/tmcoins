...
class Member extends IndexController
{
    ...
    public function __construct(App $app)
    {
        ...
        $this->member = $this->model->where('id',session('member.id'))
            ->find()->ToArray();
    ...
    public function account()
    {
        ...
        if( $this->member['email'] && $this->member['email']!=''){
            $safe = $safe+30;
            $email = 1;
        }else{
            $email = 0;
        ...
        $this->assign(['phone'=>$phone,'paypwd'=>$paypwd,'email'=>$email]);
        if($safe > 80){
...
//            $prefix=request()->post('prefix','','trim');
            ...
            if($userst){
                return $this->error(lang('wicket_page.Validate_email_isphone'));
            }
...
//            $indata['phone_time'] = time();
    ...
    public function upload_card()
    {
        ...
        $savename = \think\facade\Filesystem::disk('public')
            ->putFile($data['paths'], $data['file'], function() use ($fileName){
            return $fileName;
        });
